...
export const getEmployees = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const [rows] = await db.query<EmployeeRow[]>('SELECT e.nombre, e.apellidos, u.email, e.telefono, e.iban, e.nif, e.direccion, e.status FROM Employees e JOIN Users u ON e.user_id = u.id LIMIT ? OFFSET ?', [limit, offset]);
